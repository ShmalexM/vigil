...
export function parseSourceEvidence(value: unknown): SourceEvidence | undefined {
  ...
  const associationBasis = raw.association_basis
  if (
    raw.version !== 1
    || !KINDS.has(kind)
    ...
    || (associationBasis !== undefined && (
      provenance !== 'joined' || associationBasis !== 'exact_sequence'
    ))
  ...
  if (status !== 'available') {
    return {
      ...
      status,
      provenance,
      associationBasis: associationBasis as 'exact_sequence' | undefined,
      totalRecords: 0,
      truncated: false,
      records: [],
      rawTextTruncated: false,
