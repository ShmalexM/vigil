...
def normalize_source_evidence(
    ...
    *,
    ...
    if association_basis is not None:
        if provenance != "joined" or association_basis != "exact_sequence":
            return _invalid_evidence(kind or fallback_kind, schema_id)
        envelope["association_basis"] = association_basis
    if status != "available":
        return envelope
...
def _normalize_sequence_evidence(parsed: Mapping[str, Any]) -> Dict[str, Any]:
    """Validate an immutable exact-sequence evidence envelope."""
    status = _text(parsed.get("status"))
